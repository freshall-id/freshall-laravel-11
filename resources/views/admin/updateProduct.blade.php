...
@section('content')
    <div class="row d-flex justify-content-center g-5 pt-4">
        ...
        <div class="col-8 border border-3 rounded shadow py-3">
            ...
            <form action="{{route('update-product.action', ['product'=>$product])}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nameInput" class="form-label">Name</label>
                    <input type="text" class="form-control" id="nameInput" placeholder="Contoh : Anggur" name = "name" value="{{old('name', $product->name)}}" data-original-value="{{$product->name}}">
                </div>
                <div class="mb-3">
                    <label for="skuInput" class="form-label">SKU</label>
                    <input type="text" class="form-control" id="skuInput" placeholder="Contoh : BUAH-001" name = "sku" value="{{old('sku', $product->sku)}}" data-original-value="{{$product->sku}}">
                </div>
                <div class="mb-3">
                    <label for="categoryLabelInput" class="form-label">Choose a category label</label>
                    <select class="form-select" id="categoryLabelInput" name="categoryLabel" data-original-value="{{$product->productCategory->label}}">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="categoryNameInput" name="categoryName" data-original-value="{{$product->productCategory->id}}">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="stockInput" placeholder="Contoh : 20" name = "stock" min="1" max="99999" value="{{old('stock', $product->stock)}}" data-original-value="{{$product->stock}}"> 
                </div>
                <div class="mb-3">
                    <label for="minimumBuyInput" class="form-label">Minimum Buy</label>
                    <input type="number" class="form-control" id="minimumBuyInput" placeholder="Contoh : 1" name = "minimum_buy" min="1" max="99999" value="{{old('minimum_buy', $product->minimum_buy)}}" data-original-value="{{$product->minimum_buy}}">
                </div>
                <div class="mb-3">
                    <label for="weightInput" class="form-label">Weight/1pcs</label>
                    <input type="number" class="form-control" id="weightInput" placeholder="In Grams (G)" name = "weight" value="{{old('weight', $product->weight)}}" data-original-value="{{$product->weight}}">
                </div>
                <div class="mb-3">
                    <label for="priceInput" class="form-label">Price/1pcs</label>
                    <input type="number" class="form-control" id="priceInput" placeholder="In Rupiah (Rp)" name="price" value="{{old('price', $product->price)}}" data-original-value="{{$product->price}}">
                </div>
                <div class="mb-3">
                    <label for="descriptionInput" class="form-label">Description</label>
                    <textarea class="form-control" id="descriptionInput" rows="3" name ="description" placeholder="Contoh : Full natural" data-original-value="{{$product->description}}">{{old('description', $product->description)}}</textarea>
                </div>
