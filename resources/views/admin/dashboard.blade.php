...
@section('content')
    <div>
            ...
            <div class="col">
                <div class="ps-1 bg-warning rounded shadow h-100 d-flex align-items-center justify-content-center">
                    <div class="bg-white rounded px-3 py-4 w-100 h-100">
                        <div class="d-flex justify-content-between align-items-center h-100">
                            <div class="me-1">
                                <p class="m-0 mb-1 fw-bold" style="font-size: 14px; color: hsl(51, 100%, 46%);">TRANSACTIONS
                                    (INPROCESS)</p>
            ...
            <div class="col">
                <div class="ps-1 bg-success rounded shadow h-100 d-flex align-items-center justify-content-center">
                    <div class="bg-white rounded px-3 py-4 w-100 h-100">
                        <div class="d-flex justify-content-between align-items-center h-100">
                            <div class="me-1">
                                <p class="m-0 mb-1 text-success fw-bold" style="font-size: 14px;">Total Transactions
                                    (MONTHLY)</p>
        ...
        <div class="d-none d-md-flex flex-sm-row justify-content-between mb-3 flex-wrap">
            @foreach (['PENDING', 'INPROCESS', 'COMPLETED', 'CANCELED', 'FAILED', 'TROUBLE', 'ONHOLD'] as $status)
                <div>
                    ...
                    @else
                        <a href="{{ route('admin-dashboard.page', array_merge(request()->query(), ['selected' => $status], ['page' => 1])) }}"
        ...
        <div class="d-md-none mb-3">
            <div>
                <select class="form-select" onchange="location = this.value" aria-label="Status Selection">
                    @foreach (['PENDING', 'INPROCESS', 'COMPLETED', 'CANCELED', 'FAILED', 'TROUBLE', 'ONHOLD'] as $status)
                        <option
        ...
        <x-transaction-table :transactions="$transactions" :canUpdate="$canUpdate" :canDelete="$canDelete" />
    </div>
@endsection
