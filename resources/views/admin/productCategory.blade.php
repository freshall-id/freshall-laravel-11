...
@section('content')
    <div class="my-5">
        <h2 class="fw-bold">Manage Category Product</h2>
    </div>
    <a href="{{ route('create.productCategory.page') }}" class="mb-3 btn btn-primary">Create New Product Category</a>
    <div class="d-none d-md-flex flex-sm-row justify-content-between mb-3 flex-wrap">
        @foreach ($categoriesLabel as $label)
            <div>
                ...
                @else
                    <a href="{{ route('admin-productCategory.page', array_merge(request()->query(), ['selected' => $label], ['page' => 1])) }}"
    ...
    <div class="d-md-none mb-3">
        <div>
            <select class="form-select" onchange="location = this.value" aria-label="label Selection">
                @foreach ($categoriesLabel as $label)
                    <option
                        value="{{ $selected == $label ? '' : route('admin-productCategory.page', array_merge(request()->query(), ['selected' => $label], ['page' => 1])) }}"
    ...
    <x-product-category-table :productCategories="$productCategories" />
@endsection
