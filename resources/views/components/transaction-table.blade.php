        ...
        <tbody>
            @forelse ($transactions as $transaction)
                ...
                <tr>
                    ...
                    <td class="text-start {{ $transaction->notes ?? 'text-center' }}">
                        ...
                        <div class="d-flex flex-column gap-2 px-3">
                            ...
                            @if ($canUpdate)
                                <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modal"
                                    data-title="Edit Status Transaction"
                                    data-content="Edit status for transaction ID: {{ $transaction->id }}"
                                    data-route="{{ route('update-transaction-header.action', ['id' => $transaction->id]) }}"">
                            ...
                            @if ($canDelete)
                                <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#modal"
                                    ...
                                    data-content="Are you sure you want to delete transaction ID: {{ $transaction->id }}?"
                                    data-route="{{ route('delete-transaction-header.action', ['transactionHeader' => $transaction]) }}"">
                                                        ...
                                                        <div class="col">
                                                            ...
                                                            <select class="form-select"
                                                                ...
                                                                @foreach (['PENDING', 'INPROCESS', 'COMPLETED', 'CANCELED', 'FAILED', 'TROUBLE', 'ONHOLD'] as $option)
                                                                    <option value="{{ $option }}"
                                                                        {{ $option === $transaction->status ? 'selected' : '' }}>
                                                                        {{ $option }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
